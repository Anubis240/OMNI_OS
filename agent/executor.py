...
class _GeminiModel:
    """Thin wrapper so call sites can keep using model.generate_content(...)."""

    def __init__(self, client, model_name: str, system_instruction: str = None):
        from google.genai import types
        self._client = client
        self._model = model_name
        self._config = types.GenerateContentConfig(system_instruction=system_instruction) if system_instruction else None

    def generate_content(self, contents):
        return self._client.models.generate_content(model=self._model, contents=contents, config=self._config)


def _gemini_model(model_name: str, system_instruction: str = None) -> _GeminiModel:
    from google import genai
    client = genai.Client(api_key=_get_api_key())
    return _GeminiModel(client, model_name, system_instruction)
...
def _detect_language(text: str) -> str:
    model = _gemini_model("gemini-2.5-flash-lite")
    try:
        response = model.generate_content(
...
def _share_file(parameters: dict, dashboard, player, loop) -> str:
    # Planner steps are generated independently up front — they can't observe
    # a previous step's actual result — so share_file takes the same
    # (path shortcut, name) addressing as file_controller's create_file/write,
    # rather than a literal full path only knowable after that step runs.
    raw_path = parameters.get("path", "")
    name     = parameters.get("name", "")
    if name:
        from actions.file_controller import _resolve_path
        path = str(_resolve_path(raw_path) / name)
    else:
        path = raw_path
    ...
    url = dashboard.register_file(path)
    if not url:
    ...
    if player:
        try:
            player.write_log(f'SYS: <a href="{url}">Click here to open {filename}</a>')
        except Exception:
            pass
    if loop:
        import asyncio
        try:
            asyncio.run_coroutine_threadsafe(
                dashboard.broadcast({"type": "link", "url": url, "label": filename}), loop
            )
        except Exception:
            pass  # link is still shown on the desktop even if the phone broadcast fails
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None, dashboard=None, player=None, loop=None) -> str:

    if tool == "share_file":
        return _share_file(parameters, dashboard, player, loop)
            ...
            for step in steps:
                ...
                while attempt <= 3:
                    if cancel_flag and cancel_flag.is_set():
                        break
                    try:
                        result = _call_tool(tool, params, speak, dashboard=dashboard, player=player, loop=loop)
