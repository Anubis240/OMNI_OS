...
PLANNER_PROMPT = """You are the planning module of Omni-OS, a personal AI assistant.
...
share_file
  path: string (required) — SAME "path" value used in the file_controller step that created the file (e.g. "desktop")
  name: string (required) — SAME "name" value used in that file_controller step
  Use this as the LAST step whenever the goal asks for "a link" to open a file you created. Gives the user a clickable web link instead of a local file path.
EXAMPLES:
...
Goal: "make a 1 hour beginner English lesson plan and give me a link to open it"
Steps:

file_controller | action: create_file, path: desktop, name: english_lesson_plan.html, content: "<html>...full lesson plan content here...</html>"
share_file | path: desktop, name: english_lesson_plan.html
...
def create_plan(goal: str, context: str = "") -> dict:
    model = _gemini_model("gemini-2.5-flash-lite", PLANNER_PROMPT)
...
def replan(goal: str, completed_steps: list, failed_step: dict, error: str) -> dict:
    model = _gemini_model("gemini-2.5-flash", PLANNER_PROMPT)
