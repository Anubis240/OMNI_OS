"""dashboard/server.py — phone socket bookkeeping and pairing codes.
...
from dashboard.server import LinkHooks, PairingCode, PhoneLink
...
class PhoneSocketTests(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        self.left = 0
        self.server = PhoneLink(LinkHooks(phone_left=lambda: setattr(self, "left", self.left + 1)))
    ...
    async def test_failed_send_drops_the_socket_and_fires_once(self):
        ...
        await self.server.publish({"type": "notice", "text": "hi"})
    ...
    async def test_live_phones_still_receive_when_another_fails(self):
        ...
        await self.server.publish({"type": "notice", "text": "hi"})
        self.assertEqual(self.left, 0)
        self.assertEqual(live.sent, [{"type": "notice", "text": "hi"}])

    async def test_history_is_capped(self):
        for n in range(150):
            await self.server.publish({"n": n})
        self.assertEqual(len(self.server._backlog), 100)
        self.assertEqual(self.server._backlog[0], {"n": 50})


class PairingCodeTests(unittest.TestCase):
    """GEMZ4US Item G: a superseded code must stop working immediately — this
