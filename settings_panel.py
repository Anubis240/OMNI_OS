...
class SettingsPanel(QWidget):
    ...
    def _build_companions_section(self) -> QWidget:
        ...
        note = QLabel(
            ...
            "through its own conversation on whichever CLI backend it's assigned "
            "(Claude, Codex, OpenCode, OpenHands, Grok Build, or Blackbox) instead — "
            "voice keeps using Omni's default identity either way, since none of these "
            "Text backends expose a realtime voice API. With no companion active, "
    ...
    def refresh_companions(self) -> None:
        ...
        # green/red status banner never cleared on its own — it kept
        # showing whatever message was last generated by an action taken
        # directly in Settings, even after that action's subject (e.g. a
        # just-added companion) was later removed from a different view
        # (World Panel). Same reload point that already fixes the stale
        # companion list above; clearing the banner here keeps both in sync
        # instead of leaving a misleading leftover message on screen.
