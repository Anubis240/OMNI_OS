...
class _VisionSession:
    ...
    async def _session_loop(self) -> None:
        ...
        # Match whatever voice the main assistant is currently using, rather
        # than a hardcoded one, so the vision module doesn't sound like a
        # different assistant when it speaks.
        voice_name = getattr(self._player, "voice", None) or "Charon"
...
def screen_process(
    ...
    # Vision session connect + capture + round-trip takes a couple of
    # seconds — acknowledge immediately so the user isn't met with silence.
    if speak:
        ack = "Looking through the camera now, sir." if angle == "camera" else "Looking at your screen now, sir."
        speak(ack)
